...
class PowerQueryPivot:
    ...
    def _is_date_column(self, col):
        ...
        try:
            ...
            # Jika lebih dari 70% bisa di-parse, anggap date column
    ...
    def update_filter_value_entries(self, event=None):
        ...
        for idx in selected_indices:
            col_display = self.filter_col_lb.get(idx)
            # Hapus indikator date jika ada untuk mendapat nama asli kolom
            col = col_display.replace(" (📅 Date)", "").strip()
            
            # Cek apakah kolom ini adalah date column
            is_date_col = self._is_date_column(col)
            
            if is_date_col:
                # Untuk date column, tampilkan filter tahun
                lbl = tk.Label(self.filter_val_frame, text=f"Tahun '{col}':")
                lbl.pack(side=tk.LEFT, padx=2)
                
                # Ekstrak tahun unik dari kolom
                try:
                    years = pd.to_datetime(self.df[col], errors='coerce').dt.year.dropna().unique()
                    years = sorted([str(int(y)) for y in years if pd.notna(y)])
                except Exception:
                    years = []
                
                # Gunakan Combobox untuk pilihan tahun
                year_cb = ttk.Combobox(self.filter_val_frame, values=years, width=12, state="readonly")
                year_cb.pack(side=tk.LEFT, padx=2)
                self.filter_val_entries.append((col, year_cb, True))  # True = is date column
            else:
                # Untuk non-date column, tampilkan filter nilai biasa
                lbl = tk.Label(self.filter_val_frame, text=f"Nilai '{col}':")
                lbl.pack(side=tk.LEFT, padx=2)
                ...
                self.filter_val_entries.append((col, val_cb, False))  # False = not date column
    ...
    def apply_filter(self):
        ...
        try:
            ...
            for item in self.filter_val_entries:
                col = item[0]
                cb = item[1]
                is_date_col = item[2] if len(item) > 2 else False
                ...
                if is_date_col:
                    # Filter berdasarkan tahun untuk date columns
                    try:
                        target_year = int(val)
                        # Convert ke datetime dan ekstrak tahun
                        date_col = pd.to_datetime(filtered_df[col], errors='coerce')
                        filtered_df = filtered_df[date_col.dt.year == target_year]
                    except Exception as e:
                        messagebox.showerror("Error", f"Filter tahun gagal untuk kolom '{col}': {e}")
                        return
                else:
                    # Filter nilai biasa untuk non-date columns
    ...
    def on_sheet_selected(self, event=None):
        ...
        self.filter_col_lb.delete(0, tk.END)
        for col in cols:
            # Tambahkan indikator jika kolom adalah date column
            display_name = col
            if self._is_date_column(col):
                display_name = f"{col} (📅 Date)"
            self.filter_col_lb.insert(tk.END, display_name)
        for widget in self.filter_val_frame.winfo_children():
            widget.destroy()
        self.filter_val_entries = []
